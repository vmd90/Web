var relatorios_module = angular.module('app.relatorios', []);

relatorios_module.controller('relatoriosController', function ($scope, Service) {

	$scope.links = [];
	$scope.links.push({'name':'Top 20 usuarios', 'show': 'hidden'});
	$scope.links.push({'name':'Top 20 tweets', 'show': 'hidden'});
			...
			Service.get_top20_usuarios(d1.toUTCString(), d2.toUTCString()).then(
				...
				function (res){
					...
					for(var i=0; (i<users.length || i<20); i++){
						users[i]['qtde'] = users[i].tweets.length;
